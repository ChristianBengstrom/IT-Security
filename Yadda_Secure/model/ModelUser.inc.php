  ...
  public function update() {
    ...
    public static function retrievem() {
        ...
        try {
            ...
            $q->execute();
            while ($row = $q->fetch()) {
                $user = self::createObjectFromDatabase($row);
        ...
        public static function createObject($a) {
          ...
          //   $act = 'Deactve';
          // }
          $user = new User($a['uid'], $a['fname'], $a['lname'], $a['email'], $act);
          if (isset($a['pwd1'])) {
              $user->setPwd($a['pwd1']);
          }
          return $user;
        }
        public static function createObjectFromDatabase($a) {
          $user = new User($a['id'], $a['firstname'], $a['lastname'], $a['email'], $a['activated']);
          if (isset($a['pwd1'])) {
              $user->setPwd($a['pwd1']);
          }
          return $user;
